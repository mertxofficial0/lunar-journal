...
export default function LoginScreen({ onSuccess }: LoginScreenProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (username === "mert" && password === "1234") {
      ...
      // 3 saniye loading + dashboard arka planda preload olacak
      setTimeout(() => {
        setExiting(true); // exit animasyonu

        setTimeout(() => {
          onSuccess(); // App → stage = "app"
        }, 300);
      }, 3000);
  ...
  return (
    <div className={containerClass}>
      ...
      <div
        className="
          relative z-10 w-[380px] px-8 py-7 rounded-[26px]
          bg-white/75 border border-white/80 shadow-[0_22px_55px_rgba(148,163,184,0.55)]
          backdrop-blur-2xl flex flex-col items-center overflow-hidden
        "
      >
        {/* LOGO */}
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4 relative z-10">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="
                  ...
                  focus:bg-white focus:ring-2 focus:ring-indigo-300/70 outline-none transition text-sm
                "
              />
            </div>
          ...
          <button
            ...
            disabled={loading}
            className="
              mt-3 py-3 rounded-[16px]
              bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500
              text-white font-semibold hover:from-indigo-400 hover:via-purple-400 hover:to-pink-400
              transition flex items-center justify-center gap-2
              shadow-[0_14px_35px_rgba(168,85,247,0.55)]
        ...
        <p className="text-[11px] text-slate-400 mt-4 relative z-10">
          Demo giriş: <span className="font-mono text-slate-600">mert / 1234</span>
        </p>
      </div>
    </div>
  );
